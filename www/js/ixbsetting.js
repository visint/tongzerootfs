...
$(document).ready(function() {
	...
	$(".confirm").click(function(){
		...
		if($(".name").is(":visible")){//pppoe方式
			...
			var dns = $(".dns input").val();
			var mask = $(".select_mask span").first().data("value");
				...
				if(data.pppoe.reg == "0" ){
					if(isFirst){
						window.setTimeout(function(){
							...
							var url = regUrl + "?pppoe_username="+pppoe_username+"&acmac=" + acMac + "&ac_active_address=" + actUrl + "&factory=" + factory + "&model=" + model;
...
function getNewVersion(){
	$.ajax({
		...
		url:"http://tz.pifii.com:9091/json/version.json",
		dataType:"jsonp",
		jsonp:"callback",
