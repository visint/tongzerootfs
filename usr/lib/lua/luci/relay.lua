...
function get_encr_and_auth(ssid)
    ...
    if scan[ssid] then
        ...
        return encr, algo, scan[ssid].channel
    end
end
...
function api_relay_set(args)
    ...
    repeat
        ...
        local encr, algo, ch = get_encr_and_auth(ssid)
        ...
        if (encr==2 or encr==3) and lkey<8 then status=2 break end
        ...
        local wifidev2g = api.uci_cfg_get("idcard", "baseinfo", "wifidev2g")
        uci:set( fname, wifidev2g, "channel"   , ch    )
