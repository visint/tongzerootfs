...
function get_device_version()
   local util = require("luci.util")
    local ver = util.exec("cat /etc/sysinfo.conf | grep soft_version | cut -d '=' -f2 2> /dev/null")
   return "version " .. ver
end
...
function get_device_mac()
   local util = require("luci.util")
   local mac = util.exec("echo -n $(iwinfo ra0 info | grep Access | awk \'{print $3}\')")
   return string.upper(string.gsub(mac,":",""))
end

function get_device_arp_num()
   local util = require("luci.util")
   local mac = util.exec("cat /proc/net/arp|grep br-lan | wc -l")
   return mac - 0
...
function Api_black_mac_get( args )
 ...
 --[[   local res = {}
    ...
    if hwrite == "1" then
        luci.http.prepare_content( "application/json;;charset=utf8" )
        luci.http.write_json( ret )
    end
    return ret
end
...
--        nixio.stdclose( "oe" )
        ...
        status = luci.sys.call( "/bin/rm -rf /overlay/*&&/sbin/reboot" )
...
function ApiPassword( args )
	...
	local wifipw = api.GetParam( args, "wifiPwd", "" )
    local tempV
    repeat
	...
	if wifipw ~= "" then
		tempV = "uci set wireless.@wifi-iface[" .. "0"  .. "].key=" .. wifipw
		luci.sys.call( tempV )
                tempV = "uci set wireless.@wifi-iface[" .. "0"  .. "].encryption=psk+psk2"
                luci.sys.call( tempV )
		stat = uci:commit( "wireless" )
	end
...
--        nixio.stdclose( "oe" )
        ...
        status = luci.sys.call( "/sbin/wifi restart" )
...
function Api_wip_get(args)
    ...
    local active_url = uci:get( "pifii", "active", "url") or "http://192.168.2.1/cgi-bin/luci/admin/activeH"
    local gateway = uci:get( "network", "lan", "ipaddr")
    if gateway and "" ~= string.gsub(gateway," ","") then
       active_url = "http://"..gateway.."/cgi-bin/luci/admin/activeH"
    end
    ret.real = {
